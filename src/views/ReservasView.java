...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		this.resController = new ReservaController();
		...
		lblSimbolo.setVisible(true);
		lblSimbolo.setBounds(82, 326, 17, 25);
		lblSimbolo.setForeground(SystemColor.textHighlight);
		lblSimbolo.setFont(new Font("Roboto", Font.BOLD, 17));
		...
		txtValor.setBounds(103, 322, 43, 33);
	...
	public void guardarReserva() {
		if(txtFechaEntrada.getDate() != null && txtFechaSalida.getDate() != null && !txtValor.equals("") 
				&& !txtFormaPago.getSelectedItem().equals("")) {
			LocalDate fechaEnt = LocalDate.parse(((JTextField)txtFechaEntrada.getDateEditor().getUiComponent()).getText());
			LocalDate fechaSal = LocalDate.parse(((JTextField)txtFechaSalida.getDateEditor().getUiComponent()).getText());
			Reserva reserva = new Reserva(fechaEnt,fechaSal,txtValor.getText(), txtFormaPago.getSelectedItem().toString());
			resController.guardar(reserva);
	...
	public void calcularValor(JDateChooser fechaE, JDateChooser fechaS) {
		if(fechaE.getDate() != null && fechaS.getDate() != null) {
			if(fechaE.getDate().after(fechaS.getDate())) {
				JOptionPane.showMessageDialog(null, "La fecha seleccionada no puede ser posterior a la fecha de de entrada",
						"Error Fecha", JOptionPane.ERROR_MESSAGE);
				return;
			}
			Calendar ini = fechaE.getCalendar();
